...
<template>
	<s-layout :onShareAppMessage="shareInfo" navbar="goods">
		...
		<s-empty
			...
			text="活动不存在或已结束" :icon="sheep.$url.cdn('/static/soldout-empty.png')" showAction actionText="再逛逛"
		...
		<block v-else>
				...
				<view class="title-card detail-card ss-p-y-40 ss-p-x-20">
					<view class="ss-flex ss-row-between ss-col-center ss-m-b-18">
						<view class="price-box ss-flex ss-col-bottom">
							<image :src="sheep.$url.cdn('/img/score1.svg')" class="point-img">
							</image>
			...
			<!-- 评价 -->
			<detail-comment-card :goodsId="state.goodsInfo.id" />
			<!-- 详情 -->
			<detail-content-card :content="state.goodsInfo.description" />
				...
				<view class="buy-box ss-flex ss-col-center ss-p-r-20">
					...
					<button class="ss-reset-button btn-box ss-flex-col" @tap="state.showSelectSku = true"
						...
						<view class="price-box ss-flex">
							<image :src="sheep.$url.cdn('/img/score1.svg')"
...
<script setup>
	...
	const headerBg = sheep.$url.css('/img/score-bg.png');
	...
	const getActivity = async (id) => {
		const {
			data
		} = await PointApi.getPointActivity(id);
		activity.value = data;
		// 查询商品
