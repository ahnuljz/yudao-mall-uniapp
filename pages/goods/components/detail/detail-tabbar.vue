  ...
  <su-fixed bottom placeholder bg="bg-white">
    <view class="ui-tabbar-box">
      <view class="ui-tabbar ss-flex ss-col-center ss-row-between">
        <view v-if="collectIcon" class="detail-tabbar-item ss-flex ss-flex-col ss-row-center ss-col-center" @tap="onFavorite">
          <block v-if="modelValue.favorite">
            <image class="item-icon" :src="sheep.$url.cdn('/img/collect_1.gif')" mode="aspectFit" />
            <view class="item-title">已收藏</view>
          </block>
          <block v-else>
            <image class="item-icon" :src="sheep.$url.cdn('/img/collect_0.png')" mode="aspectFit" />
        ...
        <button
          ...
          style="color: #000; backgroundcolor: #fff; bordercolor: #fff"
          open-type="contact"
        >
          <image class="item-icon" :src="sheep.$url.cdn('/img/message.png')" mode="aspectFit" />
          <text class="item-title">客服</text>
        </button>
        // #endif

        <view v-if="shareIcon" class="detail-tabbar-item ss-flex ss-flex-col ss-row-center ss-col-center" @tap="showShareModal">
          <image class="item-icon" :src="sheep.$url.cdn('/img/share.png')" mode="aspectFit" />
