  ...
  <s-layout :bgStyle="{ color: '#FFF' }" title="收货地址">
    ...
    <su-fixed bottom placeholder>
      <view class="footer-box ss-flex ss-row-between ss-p-20">
        ...
        <button class="add-btn ss-reset-button ui-Shadow-Main" @tap="sheep.$router.go('/pages/user/address/edit')">新增收货地址</button>
      </view>
    </su-fixed>
    <s-empty v-if="state.list.length === 0 && !state.loading" text="暂无收货地址" :icon="sheep.$url.cdn('/static/data-empty.png')" />
  </s-layout>
</template>
