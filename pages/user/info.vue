    ...
    <uni-forms :model="state.model" :rules="state.rules" labelPosition="left" border class="form-box">
      ...
      <view class="ss-flex ss-row-center ss-col-center ss-p-t-60 ss-p-b-0 bg-white">
        <view class="header-box-content">
          <su-image class="content-img" isPreview :current="0" :src="state.model?.avatar" :height="160" :width="160" :radius="80" mode="scaleToFill" />
          <view class="avatar-action">
            <!-- #ifdef MP -->
            <button class="ss-reset-button avatar-action-btn" open-type="chooseAvatar" @chooseavatar="onChooseAvatar">修改</button>
            <!-- #endif -->
            <!-- #ifndef MP -->
      ...
      <view class="bg-white ss-p-x-30">
        ...
        <uni-forms-item name="nickname" label="昵称">
          <uni-easyinput v-model="state.model.nickname" type="nickname" placeholder="设置昵称" :inputBorder="false" :placeholderStyle="placeholderStyle" />
        </uni-forms-item>
      ...
      <view class="bg-white ss-m-t-14 none">
        <uni-list>
          <uni-list-item clickable @tap="sheep.$router.go('/pages/user/address/list')" title="地址管理" showArrow :border="false" class="list-border" />
        </uni-list>
      </view>
    </uni-forms>
      ...
      <view class="account-list ss-flex ss-row-between">
        ...
        <view class="ss-flex ss-col-center">
          ...
          <view class="bind-box ss-m-l-20">
            <button v-if="state.thirdInfo.openid" class="ss-reset-button relieve-btn" @tap="unBindThirdOauth">解绑</button>
