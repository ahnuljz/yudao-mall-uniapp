<template>
  <s-layout class="chat-wrap" :title="!isReconnecting ? '连接客服成功' : '会话重连中'" navbar="inner">
    ...
    <MessageList ref="messageListRef">
      <template #bottom>
        <message-input v-model="chat.message" @on-tools="onTools" @send-message="onSendMessage"></message-input>
      </template>
    </MessageList>
    <!--  聊天工具  -->
    <tools-popup :show-tools="chat.showTools" :tools-mode="chat.toolsMode" @close="handleToolsClose" @on-emoji="onEmoji" @image-select="onSelect" @on-show-select="onShowSelect">
      <message-input v-model="chat.message" @on-tools="onTools" @send-message="onSendMessage"></message-input>
    </tools-popup>
    <!--  商品订单选择  -->
...
import { db, websocketPath } from '@/sheep/config';
...
async function onSendMessage() {
  ...
  try {
    const data = {
      ...
      content: JSON.stringify({ text: chat.message }),
    };
