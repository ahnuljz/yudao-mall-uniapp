      ...
      <view class="modal-content ss-flex-1">
        ...
        <radio-group @change="onTapPay">
          <label class="pay-type-item" v-for="item in state.payMethods" :key="item.title">
            <view class="pay-item ss-flex ss-col-center ss-row-between ss-p-x-30 border-bottom" :class="{ 'disabled-pay-item': item.disabled }">
              <view class="ss-flex ss-col-center">
                <image class="pay-icon" v-if="item.disabled" :src="sheep.$url.cdn('/img/cod_disabled.png')" mode="aspectFit" />
                <image class="pay-icon" v-else :src="sheep.$url.cdn(item.icon)" mode="aspectFit" />
  ...
  if (state.payment === 'wallet') {
    uni.showModal({
      ...
      success: function (res) {
        if (res.confirm) {
          sheep.$platform.pay(state.payment, state.orderType, state.orderInfo.orderId);
        }
      },
    });
  } else {
...
function checkPayStatus() {
  if (state.orderInfo.status === 'SUCCESS' || state.orderInfo.status === 'CLOSED') {
    // 支付成功
    state.payStatus = 2;
    // 跳转回支付成功页
    uni.showModal({
      ...
      showCancel: false,
      success: function () {
        goPayResult(state.orderInfo.orderId, state.orderType);
      },
    });
    return;
  }
  if (state.orderInfo.status === 'TIME_OUT') {
    // 支付关闭
    state.payStatus = -1;
    return;
  }
...
async function setPayMethods() {
  state.payMethods = getPayMethods(['wx_lite', 'alipay_wap', 'wallet']);
...
onLoad((options) => {
  if (sheep.$platform.name === 'WechatOfficialAccount' && sheep.$platform.os === 'ios' && !sheep.$platform.landingPage.includes('pages/pay/index')) {
    location.reload();
    return;
  }
  // 获得支付订单信息
