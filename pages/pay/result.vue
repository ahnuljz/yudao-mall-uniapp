  ...
  <s-layout :bgStyle="{ color: '#FFF' }" title="支付结果">
    <view class="pay-result-box ss-flex-col ss-row-center ss-col-center">
      ...
      <image
        ...
        :src="sheep.$url.cdn('/img/order_pay_success.gif')"
      />
      <image
        class="pay-img ss-m-b-30"
        v-if="['failed', 'closed'].includes(payResult)"
        :src="sheep.$url.cdn('/img/order_paty_fail.gif')"
      />
      ...
      <view class="btn-box ss-flex ss-row-center ss-m-t-50">
        <button class="back-btn ss-reset-button" @tap="sheep.$router.go('/pages/index/index')"> 返回首页 </button>
        <button
          class="check-btn ss-reset-button"
          v-if="payResult === 'failed'"
          @tap="sheep.$router.redirect('/pages/pay/index', { id: state.id, orderType: state.orderType })"
        >
          重新支付
        </button>
        <button class="check-btn ss-reset-button" v-if="payResult === 'success'" @tap="onOrder"> 查看订单 </button>
        <button
          class="check-btn ss-reset-button"
      ...
      <view class="subscribe-box ss-flex ss-m-t-44" v-if="showSubscribeBtn && state.orderType === 'goods'">
        <image class="subscribe-img" :src="sheep.$url.cdn('/img/cargo.png')" />
...
<script setup>
  ...
  async function getOrderInfo(id) {
    ...
    // 2.1 情况三一：未支付，且轮询次数小于5次，则继续轮询
    if (state.counter < 5 && state.result === 'unpaid') {
      setTimeout(() => {
        getOrderInfo(id);
      }, 2000);
    }
    // 2.2 情况二：超过5次检测才判断为支付失败
    if (state.counter >= 5) {
      state.result = 'failed';
      showRepayModal();
    }
