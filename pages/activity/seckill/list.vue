...
<template>
  <s-layout :bgStyle="{ color: 'rgb(245,28,19)' }" navbar="inner">
    ...
    <view class="header" v-if="activeTimeConfig?.sliderPicUrls?.length > 0">
      <swiper indicator-dots="true" autoplay="true" :circular="true" interval="3000" duration="1500" indicator-color="rgba(255,255,255,0.6)" indicator-active-color="#fff">
    ...
    <view class="flex align-center justify-between ss-p-25">
      ...
      <view class="time-icon">
        <image class="ss-w-100 ss-h-100" :src="sheep.$url.cdn('/img/priceTag.png')" />
      </view>
    ...
    <view class="list-content">
      ...
      <!-- 活动列表 -->
      <scroll-view class="scroll-box" :style="{ height: pageHeight + 'rpx' }" scroll-y="true" :scroll-with-animation="false" :enable-back-to-top="true">
        <view class="goods-box ss-m-b-20" v-for="activity in activityList" :key="activity.id">
          <s-goods-column size="lg" :data="{ ...activity, price: activity.seckillPrice }" :goodsFields="goodsFields" :seckillTag="true">
            <!-- 抢购进度 -->
            <template #activity>
              <view class="limit">
                限量
            ...
            <template #cart>
              ...
              <button :class="['ss-reset-button cart-btn', { disabled: activeTimeConfig?.status === TimeStatusEnum.END }]" v-else>
                <span>已结束</span>
              </button>
            </template>
          </s-goods-column>
...
const headerBg = sheep.$url.css('/img/seckill-header.png');
