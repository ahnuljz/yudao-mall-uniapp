  ...
  <s-layout class="activity-wrap" :title="state.activityInfo.title">
    ...
    <su-sticky bgColor="#fff">
      <view class="ss-flex ss-col-top tip-box">
        ...
        <image class="activity-left-image" :src="sheep.$url.cdn('/static/activity-left.png')" />
        <image class="activity-right-image" :src="sheep.$url.cdn('/static/activity-right.png')" />
      </view>
    </su-sticky>
    ...
    <view class="ss-flex ss-flex-wrap ss-p-x-20 ss-m-t-20 ss-col-top">
      <view class="goods-list-box">
        <view class="left-list" v-for="item in state.leftGoodsList" :key="item.id">
          <s-goods-column class="goods-md-box" size="md" :data="item" @click="sheep.$router.go('/pages/goods/index', { id: item.id })" @getHeight="mountMasonry($event, 'left')">
      ...
      <view class="goods-list-box">
        <view class="right-list" v-for="item in state.rightGoodsList" :key="item.id">
          <s-goods-column class="goods-md-box" size="md" :data="item" @click="sheep.$router.go('/pages/goods/index', { id: item.id })" @getHeight="mountMasonry($event, 'right')">
            <template v-slot:cart>
