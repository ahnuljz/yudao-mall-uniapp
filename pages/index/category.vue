  ...
  <s-layout :bgStyle="{ color: '#fff' }" tabbar="/pages/index/category" title="分类">
    <view class="s-category">
      <view class="three-level-wrap ss-flex ss-col-top" :style="[{ height: pageHeight + 'px' }]">
        ...
        <!-- 商品分类（右） -->
        <scroll-view class="goods-list-box" scroll-y :style="[{ height: pageHeight + 'px' }]" v-if="state.categoryList?.length" @scrolltolower="handleScrollToLower">
          <image v-if="state.categoryList[state.activeMenu].image" class="banner-img" :src="sheep.$url.cdn(state.categoryList[state.activeMenu].image)" mode="widthFix" />
...
const state = reactive({
  style: 'second_one', // first_one（一级 - 样式一）, first_two（一级 - 样式二）, second_one（二级）
  categoryList: [], // 商品分类树
...
async function getList() {
  ...
  const arr = handleTree(data, 'id', 'pid', 'children')[0]['children'];
  const second = !!arr[0]?.children;
  state.categoryList = arr;
  state.style = second ? 'second_one' : 'first_one';
}
...
async function getGoodsList() {
  ...
  // 合并列表
  state.pagination.list = _.concat(state.pagination.list, res.data.records);
  state.pagination.total = res.data.totalRow;
  state.style = res.data.totalRow > 6 ? 'first_two' : 'first_one';
