...
<template>
	<s-layout title="我的团队" :class="state.scrollTop ? 'team-wrap' : ''" navbar="inner">
      ...
      >
          ...
          <view class="iconfont icon-tuandui" />
			...
			<view style="padding: 0 20rpx">
				...
				<view class="search acea-row row-between-wrapper"
					...
					<image :src="sheep.$url.cdn('/theme/red/search.png')" mode="" style="width: 60rpx; height: 64rpx"
						@click="submitForm" />
				</view>
				<view class="list">
					...
					<block v-for="(item, index) in state.pagination.list" :key="index">
						<view class="item acea-row row-between-wrapper" style="display: flex">
							...
							<view class="right"
								...
								<view>
									<text class="num">{{ item.orderCount || 0 }}
									</text>
									单
								</view>
								<view>
									<text class="num">{{ fen2yuan(item.brokeragePrice) || 0 }}
									</text>
									元
								</view>
							</view>
		...
		<s-empty v-if="state.pagination.total === 0" :icon="sheep.$url.cdn('/static/data-empty.png')" text="暂无团队信息">
		</s-empty> -->
	</s-layout>
</template>
