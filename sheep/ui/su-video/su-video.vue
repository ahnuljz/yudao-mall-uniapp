<template>
  <view class="ui-video-wrap">
    <!-- #ifndef APP-PLUS -->
    <video :id="`sVideo${uid}`" class="radius" :style="[{ height: height + 'rpx' }]" :src="src" controls
      object-fit="contain" :enable-progress-gesture="state.enableProgressGesture" :initial-time="initialTime"
      x5-video-player-type="h5" x-webkit-airplay="allow" webkit-playsinline="true" @error="videoErrorCallback"
      @timeupdate="timeupdate" @play="play" @pause="pause" @ended="end" :poster="poster" :autoplay="autoplay">
    </video>
    <!-- #endif -->
    <!-- #ifdef APP-PLUS -->
    <dom-video ref="domVideo" class="radius" :style="[{ height: height + 'rpx' }]" object-fit="contain" :controls="true"
      :show-progress="false" :show-fullscreen-btn="false" :show-play-btn="false" :show-bottom-progress="false"
      :autoplay="false" :src="src" @play="play" @pause="pause" @ended="end" />
    <!-- #endif -->
    <!-- <view
      v-show="!state.isplay"
