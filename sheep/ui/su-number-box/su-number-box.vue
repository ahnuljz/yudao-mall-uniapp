<template>
  <view class="uni-numbox">
    ...
    ></text>
    <!-- </view> -->
    <input :disabled="disabled" @focus="_onFocus" @blur="_onBlur" class="uni-numbox__value" type="number" v-model="inputValue" :style="{ color }" />
...
export default {
  name: 'UniNumberBox',
  emits: ['change', 'input', 'update:modelValue', 'blur', 'focus', 'increase', 'decrease'],
  props: {
    value: {
      type: [Number, String],
      default: 1,
  ...
  methods: {
    _calcValue(type) {
      ...
      if (type === 'plus') {
        this.$emit('increase', +this.inputValue);
      }
      if (type === 'minus') {
        this.$emit('decrease', +this.inputValue);
      }
    },
    _getDecimalScale() {
      let scale = 1;
