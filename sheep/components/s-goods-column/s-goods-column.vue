...
<template>
  <view class="ss-goods-wrap">
    ...
    <view v-if="size === 'xs'" class="xs-goods-card ss-flex ss-col-stretch" :style="[elStyles]" @tap="onClick">
      ...
      <view v-if="goodsFields.title?.show || goodsFields.name?.show || goodsFields.price?.show" class="xs-goods-content ss-flex-col ss-row-around">
        <view
        ...
        <view class="iconBox" v-if="data.promotionType > 0 || data.rewardActivity">
          ...
          <view class="card2" v-for="item in getRewardActivityRuleItemDescriptions(data.rewardActivity).slice(0, 1)" :key="item">
            {{ item }}
          </view>
        </view>
        <view v-if="goodsFields.price?.show" class="xs-goods-price font-OPPOSANS" :style="[{ color: goodsFields.price.color }]">
          <!-- 活动价格 -->
    ...
    <view v-if="size === 'sm'" class="sm-goods-card ss-flex-col" :style="[elStyles]" @tap="onClick">
      ...
      >
        ...
        <view v-if="goodsFields.price?.show" class="sm-goods-price font-OPPOSANS" :style="[{ color: goodsFields.price.color }]">
          <!-- 活动价格 -->
          <view class="ss-flex" v-if="data.activityType && data.activityType === PromotionActivityTypeEnum.POINT.type">
            <image :src="sheep.$url.cdn('/img/score1.svg')" class="point-img"></image>
            <text class="point-text ss-m-r-16">
    ...
    <view v-if="size === 'md'" class="md-goods-card ss-flex-col" :style="[elStyles]" @tap="onClick">
      ...
      <view class="md-goods-content ss-flex-col ss-row-around ss-p-b-20 ss-p-t-20 ss-p-x-16" :id="elId">
        ...
        <view class="ss-flex ss-col-bottom">
          <view v-if="goodsFields.price?.show" class="md-goods-price ss-m-t-16 font-OPPOSANS ss-m-r-10" :style="[{ color: goodsFields.price.color }]">
            <!-- 活动价格 -->
            <view class="ss-flex" v-if="data.activityType && data.activityType === PromotionActivityTypeEnum.POINT.type">
              <image :src="sheep.$url.cdn('/img/score1.svg')" class="point-img"></image>
              <text class="point-text ss-m-r-16">
    ...
    <view v-if="size === 'lg'" class="lg-goods-card ss-flex ss-col-stretch" :style="[elStyles]" @tap="onClick">
      ...
      <view class="lg-goods-content ss-flex-1 ss-flex-col ss-row-between ss-p-b-10 ss-p-t-20">
        <view>
          <view v-if="goodsFields.title?.show || goodsFields.name?.show" class="lg-goods-title ss-line-2" :style="[{ color: titleColor }]">
            {{ data.title || data.name }}
          </view>
          <view
            v-if="goodsFields.subtitle?.show || goodsFields.introduction?.show"
    ...
    <view v-if="size === 'sl'" class="sl-goods-card ss-flex-col" :style="[elStyles]" @tap="onClick">
      ...
      <image class="sl-img-box" :src="sheep.$url.cdn(data.picUrl)" mode="aspectFill" />
      <view class="sl-goods-content">
        <view>
          <view v-if="goodsFields.title?.show || goodsFields.name?.show" class="sl-goods-title ss-line-1" :style="[{ color: titleColor }]">
            {{ data.routeName }}
          </view>
          <view
            v-if="goodsFields.subtitle?.show || goodsFields.introduction?.show"
          ...
          <view v-if="goodsFields.price?.show" class="ss-flex ss-col-bottom font-OPPOSANS">
            <view class="sl-goods-price ss-m-r-12" :style="[{ color: goodsFields.price.color }]">
              ...
              <template v-else>
                ...
                <text v-if="data.promotionPrice > 0">{{ fen2yuan(data.promotionPrice) }}</text>
                <text v-else>
                  {{ fen2yuan(data.adultCost) }}
                </text>
              </template>
            </view>
            <view
              v-if="(goodsFields.original_price?.show || goodsFields.marketPrice?.show) && data.original_price > 0"
              class="goods-origin-price ss-m-t-16 font-OPPOSANS ss-flex"
              :style="[{ color: originPriceColor }]"
            >
              <text class="price-unit ss-font-20">{{ priceUnit }}</text>
              <view class="ss-m-l-8">{{ fen2yuan(data.adultCost) }}</view>
            </view>
          </view>
...
import { fen2yuan, formatExchange, formatSales, formatStock, getRewardActivityRuleItemDescriptions } from '@/sheep/hooks/useGoods';
...
const salesAndStock = computed(() => {
  ...
  if (props.goodsFields.salesCount?.show) {
    if (props.data.activityType && props.data.activityType === PromotionActivityTypeEnum.POINT.type) {
      ...
      text.push(formatSales(props.data.sales_show_type, props.data.saleCount));
    }
  }
  if (props.goodsFields.stock?.show) {
    if (props.data.activityType && props.data.activityType === PromotionActivityTypeEnum.POINT.type) {
      ...
      text.push(formatStock(props.data.stock_show_type, props.data.stockCount));
    }
  }
  return text.join(' | ');
});
